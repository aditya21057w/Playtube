        ...
        public void AddToWatchLater(VideoDataObject video, int count = 0)
        {
            try
            {
                if (UserDetails.IsLogin)
                {
                    ...
                    PollyController.RunRetryPolicyFunction(new List<Func<Task>> { () => RequestsAsync.Video.AddToWatchLaterVideosAsync(video.Id) });
                    Toast.MakeText(Activity, Activity.GetText(Resource.String.Lbl_Video_added), ToastLength.Short)?.Show();
                }
                else
                {
