...
namespace PlayTube.Helpers.Ads
{
    public static class AdsGoogle
    {
        ...
        private class AdMobInterstitial : AnjoInterstitialLoadCallback
        {
            ...
            public void Show(Activity context)
            {
                try
                {
                    ...
                    var requestBuilder = new AdManagerAdRequest.Builder().Build();
            ...
            public void BindAdMobNative(Activity act)
            {
                try
                {
                    if (AppTools.GetStatusAds() && AppSettings.ShowAdMobNative)
                    {
                        ...
                        AdLoader.LoadAds(new AdManagerAdRequest.Builder().Build(), 5);
                    }
                }
                catch (Exception e)
                {
        ...
        public class AdMobRewardedVideo : AnjoRewardedLoadCallback
        {
            ...
            public void ShowAd(Activity context)
            {
                try
                {
                    ...
                    AdRequest adRequest = new AdManagerAdRequest.Builder().Build();
        ...
        public static AdManagerAdView InitBannerAdView(Activity activity, LinearLayout adContainer, RecyclerView mRecycler, AdSize adSize = null)
        {
            try
            {
                if (AppTools.GetStatusAds() && AppSettings.ShowAdMobBanner)
                {
                    AdManagerAdView mAdView = new AdManagerAdView(activity);

                    if (adSize != null)
                        mAdView.SetAdSizes(adSize);
                    else
                        mAdView.SetAdSizes(GetFullWidthAdaptiveSize(activity), AdSize.Banner);
        ...
        public static void InitAdView(AdManagerAdView mAdView, RecyclerView mRecycler)
        {
            try
            {
        ...
        private class MyAdListener : AdListener
        {
            ...
            public MyAdListener(AdManagerAdView mAdView, RecyclerView mRecycler)
            {
                MAdView = mAdView;
                MRecycler = mRecycler;
            }
        ...
        private class AppOpenManager : AnjoAppOpenLoadCallback
        {
            ...
            public void ShowAdIfAvailable()
            {
                try
                {
                    AdRequest request = new AdManagerAdRequest.Builder().Build();
